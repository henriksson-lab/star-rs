...
pub fn transcript_transformgenome_l4_transcript_transformgenome(
...

    {
        ...
        for ib in 1..n_b {
            ...
            let gap_r = a.exons[n_b1][EX_R]
                .wrapping_sub(a.exons[n_b1 - 1][EX_R])
                .wrapping_sub(a.exons[n_b1 - 1][EX_L]);
            let gap_g = a.exons[n_b1][EX_G]
                .wrapping_sub(a.exons[n_b1 - 1][EX_G])
                .wrapping_sub(a.exons[n_b1 - 1][EX_L]);

            if gap_r == gap_g {
                a.exons[n_b1 - 1][EX_L] += a.exons[n_b1][EX_L] + gap_r;
            } else {
                let min_gap = gap_r.min(gap_g);
                if min_gap > 0 {
                    a.exons[n_b1][EX_L] += min_gap;
                    a.exons[n_b1][EX_G] -= min_gap;
                    a.exons[n_b1][EX_R] -= min_gap;
                }
                n_b1 += 1;
            }
        }
    ...
    for ia in 0..a.n_exons.saturating_sub(1) as usize {
        ...
        let j_s = (a.exons[ia][EX_G] + a.exons[ia][EX_L]) as u64;
        let j_e = (a.exons[ia + 1][EX_G] - 1) as u64;
        ...
        if sjdb_ind >= 0 {
            ...
            if gen_out.sjdb_motif[sjdb_ind] == 0 {
                let shift = gen_out.sjdb_shift_left[sjdb_ind] as u64;
            ...
            if gap_r > 0 {
                a.canon_sj[ia] = -2;
            } else if gap_g >= gen_out.align_intron_min as u64 {
                a.canon_sj[ia] = 0;
                let g = &gen_out.g;
                let js = j_s as usize;
                let je = j_e as usize;
